...
import SubscriptionTransactionInfoTable from "./SubscriptionTransactionInfoTable";
import html2canvas from "html2canvas";
import jsPDF from "jspdf";
import { ipfsUploader } from "@/src/ipfsUploader";
import {
  AddBlockChainType,
  SubscriptionTransactionReturnType,
} from "@/types/SubscriptionTransaction";
...
function SubscriptionTransaction() {
  const [subscriptionTransaction, setSubscriptionTransaction] =
    useState<SubscriptionTransactionReturnType | null>(null);
  const subscriptionTransactionTableRef = useRef(null);
  ...
  const [
    isSubscriptionTransactionInfoExists,
    setIsSubscriptionTransactionInfoExists,
  ] = useState(true);
  ...
  useEffect(() => {
    const getSubscriptionTransaction = async () => {
      const res = await fetch("/api/user/subscriptionTransaction/getByUserId", {
        method: "GET",
        headers: {
      ...
      if (!res.ok) {
        ...
        if (data.subscriptionTransaction) {
          setSubscriptionTransaction(data.subscriptionTransaction);
          if (data.subscriptionTransaction?.ipfsHash) {
            setIsButtonVisible(false);
            // setIPFSURL(`https://ipfs.io/ipfs/${data.subscriptionTransaction.ipfsHash}`);
            setIPFSURL(
              `https://neu-my-devlet.s3.eu-north-1.amazonaws.com/${data.subscriptionTransaction.ipfsHash}`
            );
          }
        } else {
          setIsSubscriptionTransactionInfoExists(false);
        }
      }
    };
  ...
  const addIPFS = async () => {
    const input = subscriptionTransactionTableRef.current;
    if (input)
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (ipfsHash) {
      const addBlockchainData: AddBlockChainType = {
        id: subscriptionTransaction?._id,
        smartContract: smartContract,
        ipfsHash,
      };

      const res = await fetch(
        "/api/user/subscriptionTransaction/addBlockchain",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!res.ok) {
        ...
        setOpenModalForAddBlockchain(false);
        setSubscriptionTransaction(data.eb);
        setIsLoading(false);
        setIsButtonVisible(false);
  ...
  return (
    ...
    >
      {isSubscriptionTransactionInfoExists ? (
        <>
          {subscriptionTransaction?.subscriptionTransactionInfos ? (
            <Box
              sx={{
            ...
            >
              <SubscriptionTransactionInfoTable
                {...{
        ...
        >
          ...
          >
            You don&apos;t have any Subscription Transaction info
          </Typography>
        </Box>
      )}
