...
import MilitaryInfoTable from './MilitaryInfoTable';
import html2canvas from 'html2canvas';
import jsPDF from 'jspdf';
import { ipfsUploader } from '@/src/ipfsUploader';
import {
  AddBlockChainType,
  MilitaryBackgroundReturnType,
} from '@/types/MilitaryBackground';
...
function MilitaryBackground() {
  const [militaryBackground, setMilitaryBackground] =
    useState<MilitaryBackgroundReturnType | null>(null);
  const militaryTableRef = useRef(null);
  ...
  const [isMilitaryInfoExists, setIsMilitaryInfoExists] = useState(true);
  ...
  useEffect(() => {
    const getMilitaryBackground = async () => {
      const res = await fetch('/api/user/militaryBackground/getByUserId', {
        method: 'GET',
        headers: {
      ...
      if (!res.ok) {
        ...
        if (data.militaryBackground) {
          setMilitaryBackground(data.militaryBackground);
          if (data.militaryBackground?.ipfsHash) {
            setIsButtonVisible(false);
            // setIPFSURL(`https://ipfs.io/ipfs/${data.militaryBackground.ipfsHash}`);
            setIPFSURL(
              `https://neu-my-devlet.s3.eu-north-1.amazonaws.com/${data.militaryBackground.ipfsHash}`
            );
          }
        } else {
          setIsMilitaryInfoExists(false);
        }
      }
    };
    getMilitaryBackground();
  }, []);

  const addIPFS = async () => {
    const input = militaryTableRef.current;
    if (input)
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (ipfsHash) {
      const addBlockchainData: AddBlockChainType = {
        id: militaryBackground?._id,
        smartContract: smartContract,
        ipfsHash,
      };

      const res = await fetch('/api/user/militaryBackground/addBlockchain', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${getAccessTokenFromLocalStorage()}`,
  ...
  return (
    ...
    >
      {isMilitaryInfoExists ? (
        <>
          {militaryBackground?.militaryInfos ? (
            <Box
              sx={{
            ...
            >
              <MilitaryInfoTable
                {...{ militaryBackground, militaryTableRef }}
              />
              {isButtonVisible ? (
                <Button
        ...
        >
          ...
          >
            You don&apos;t have any military info
          </Typography>
        </Box>
      )}
