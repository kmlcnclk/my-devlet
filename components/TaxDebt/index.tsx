...
import { AddBlockChainType, TaxDebtReturnType } from '@/types/TaxDebt';
...
function TaxDebt() {
  ...
  const [isTaxDebtInfoExists, setIsTaxDebtInfoExists] = useState(true);
  ...
  useEffect(() => {
    const getTaxDebt = async () => {
      const res = await fetch('/api/user/taxDebt/getByUserId', {
        method: 'GET',
        headers: {
      ...
      if (!res.ok) {
        ...
        if (data.taxDebt) {
          ...
          if (data.taxDebt?.ipfsHash) {
            setIsButtonVisible(false);
            // setIPFSURL(`https://ipfs.io/ipfs/${data.taxDebt.ipfsHash}`);
            setIPFSURL(
              `https://neu-my-devlet.s3.eu-north-1.amazonaws.com/${data.taxDebt.ipfsHash}`
            );
          }
        } else {
          setIsTaxDebtInfoExists(false);
        }
      }
    };
    getTaxDebt();
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (ipfsHash) {
      ...
      const res = await fetch('/api/user/taxDebt/addBlockchain', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${getAccessTokenFromLocalStorage()}`,
      ...
      {isTaxDebtInfoExists ? (
        <>
          {taxDebt?.taxDebtInfos ? (
            ...
            >
              <TaxDebtInfoTable {...{ taxDebt, taxDebtTableRef }} />
              {isButtonVisible ? (
                <Button
        ...
        >
          ...
          >
            You don&apos;t have any taxDebt info
          </Typography>
        </Box>
      )}
