...
import FamilyTreeInfoTable from './FamilyTreeInfoTable';
import html2canvas from 'html2canvas';
import jsPDF from 'jspdf';
import { ipfsUploader } from '@/src/ipfsUploader';
import {
  AddBlockChainType,
  FamilyTreeBackgroundReturnType,
} from '@/types/FamilyTreeBackground';
...
function FamilyTreeBackground() {
  const [familyTreeBackground, setFamilyTreeBackground] =
    useState<FamilyTreeBackgroundReturnType | null>(null);
  const familyTreeTableRef = useRef(null);
  ...
  const [isFamilyTreeInfoExists, setIsFamilyTreeInfoExists] = useState(true);
  ...
  useEffect(() => {
    const getFamilyTreeBackground = async () => {
      const res = await fetch('/api/user/familyTreeBackground/getByUserId', {
        method: 'GET',
        headers: {
      ...
      if (!res.ok) {
        ...
        if (data.familyTreeBackground) {
          setFamilyTreeBackground(data.familyTreeBackground);
          if (data.familyTreeBackground?.ipfsHash) {
            setIsButtonVisible(false);
            // setIPFSURL(`https://ipfs.io/ipfs/${data.familyTreeBackground.ipfsHash}`);
            setIPFSURL(
              `https://neu-my-devlet.s3.eu-north-1.amazonaws.com/${data.familyTreeBackground.ipfsHash}`
            );
          }
        } else {
          setIsFamilyTreeInfoExists(false);
        }
      }
    };
    getFamilyTreeBackground();
  }, []);

  const addIPFS = async () => {
    const input = familyTreeTableRef.current;
    if (input)
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (ipfsHash) {
      const addBlockchainData: AddBlockChainType = {
        id: familyTreeBackground?._id,
        smartContract: smartContract,
        ipfsHash,
      };

      const res = await fetch('/api/user/familyTreeBackground/addBlockchain', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${getAccessTokenFromLocalStorage()}`,
      ...
      if (!res.ok) {
        ...
        setOpenModalForAddBlockchain(false);
        setFamilyTreeBackground(data.eb);
        setIsLoading(false);
        setIsButtonVisible(false);
  ...
  return (
    ...
    >
      {isFamilyTreeInfoExists ? (
        <>
          {familyTreeBackground?.familyTreeInfos ? (
            <Box
              sx={{
            ...
            >
              <FamilyTreeInfoTable
                {...{ familyTreeBackground, familyTreeTableRef }}
              />
              {isButtonVisible ? (
                <Button
        ...
        >
          ...
          >
            You don&apos;t have any familyTree info
          </Typography>
        </Box>
      )}
