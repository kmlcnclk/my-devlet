...
import PlaceOfResidenceInfoTable from "./PlaceOfResidenceInfoTable";
import html2canvas from "html2canvas";
import jsPDF from "jspdf";
import { ipfsUploader } from "@/src/ipfsUploader";
import {
  AddBlockChainType,
  PlaceOfResidenceReturnType,
} from "@/types/PlaceOfResidence";
...
function PlaceOfResidence() {
  const [placeOfResidence, setPlaceOfResidence] =
    useState<PlaceOfResidenceReturnType | null>(null);
  const placeOfResidenceTableRef = useRef(null);
  ...
  const [isPlaceOfResidenceInfoExists, setIsPlaceOfResidenceInfoExists] =
    useState(true);
  ...
  useEffect(() => {
    const getPlaceOfResidence = async () => {
      const res = await fetch("/api/user/placeOfResidence/getByUserId", {
        method: "GET",
        headers: {
      ...
      if (!res.ok) {
        ...
        if (data.placeOfResidence) {
          ...
          if (data.placeOfResidence?.ipfsHash) {
            setIsButtonVisible(false);
            // setIPFSURL(`https://ipfs.io/ipfs/${data.placeOfResidence.ipfsHash}`);
            setIPFSURL(
              `https://neu-my-devlet.s3.eu-north-1.amazonaws.com/${data.placeOfResidence.ipfsHash}`
            );
          }
        } else {
          setIsPlaceOfResidenceInfoExists(false);
        }
      }
    };
    getPlaceOfResidence();
  ...
  const addIPFS = async () => {
    const input = placeOfResidenceTableRef.current;
    if (input)
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (ipfsHash) {
      const addBlockchainData: AddBlockChainType = {
        id: placeOfResidence?._id,
        smartContract: smartContract,
        ipfsHash,
      };

      const res = await fetch("/api/user/placeOfResidence/addBlockchain", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${getAccessTokenFromLocalStorage()}`,
  ...
  return (
    ...
    >
      {isPlaceOfResidenceInfoExists ? (
        <>
          {placeOfResidence?.placeOfResidenceInfos ? (
            <Box
              sx={{
            ...
            >
              <PlaceOfResidenceInfoTable
                {...{ placeOfResidence, placeOfResidenceTableRef }}
              />
              {isButtonVisible ? (
                <Button
        ...
        >
          ...
          >
            You don&apos;t have any Place of Residence info
          </Typography>
        </Box>
      )}
