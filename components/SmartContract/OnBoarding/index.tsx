...
import { getSmartContracts } from '@/store/slices/mySmartContractsSlice';
...
const OnBoarding: React.FC<Props> = ({}: Props) => {
  ...
  useEffect(() => {
    const saveSmartContract = async () => {
      ...
      const createRes = await fetch('/api/user/smart-contract/create', {
        method: 'POST',
        headers: {
      ...
      if (!createRes.ok) {
        ...
        const res = await fetch('/api/user/smart-contract/getAllWithUserID', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${getAccessTokenFromLocalStorage()}`,
        ...
        if (!res.ok) {
          ...
          dispatch(
            getSmartContracts(getAccessTokenFromLocalStorage() as string)
          );
        }
      }
    };
