...
function CriminalRecord() {
  ...
  useEffect(() => {
    const getCriminalRecord = async () => {
      const res = await fetch('/api/user/criminalRecord/getByUserId', {
        method: 'GET',
        headers: {
      ...
      if (!res.ok) {
        ...
        if (data.criminalRecord) {
          ...
          if (data.criminalRecord?.ipfsHash) {
            setIsButtonVisible(false);
            // setIPFSURL(`https://ipfs.io/ipfs/${data.criminalRecord.ipfsHash}`);
            setIPFSURL(
              `https://neu-my-devlet.s3.eu-north-1.amazonaws.com/${data.criminalRecord.ipfsHash}`
            );
          }
        } else {
          setIsCriminalRecordInfoExists(false);
        }
      }
    };
    getCriminalRecord();
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (ipfsHash) {
      ...
      const res = await fetch('/api/user/criminalRecord/addBlockchain', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${getAccessTokenFromLocalStorage()}`,
  ...
  return (
    ...
    >
      {isCriminalRecordInfoExists ? (
        <>
          {criminalRecord?.criminalRecordInfos ? (
            <Box
              sx={{
        ...
        >
          ...
          >
            You don&apos;t have any criminalRecord info
          </Typography>
        </Box>
      )}
