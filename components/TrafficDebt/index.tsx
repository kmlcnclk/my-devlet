...
import TrafficDebtInfoTable from "./TrafficDebtInfoTable";
import html2canvas from "html2canvas";
import jsPDF from "jspdf";
import { ipfsUploader } from "@/src/ipfsUploader";
import { AddBlockChainType, TrafficDebtReturnType } from "@/types/TrafficDebt";
...
function TrafficDebt() {
  const [trafficDebt, setTrafficDebt] = useState<TrafficDebtReturnType | null>(
    null
  );
  const trafficDebtTableRef = useRef(null);
  ...
  const [isTrafficDebtInfoExists, setIsTrafficDebtInfoExists] = useState(true);
  ...
  useEffect(() => {
    const getTrafficDebt = async () => {
      const res = await fetch("/api/user/trafficDebt/getByUserId", {
        method: "GET",
        headers: {
      ...
      if (!res.ok) {
        ...
        if (data.trafficDebt) {
          setTrafficDebt(data.trafficDebt);
          if (data.trafficDebt?.ipfsHash) {
            setIsButtonVisible(false);
            // setIPFSURL(`https://ipfs.io/ipfs/${data.trafficDebt.ipfsHash}`);
            setIPFSURL(
              `https://neu-my-devlet.s3.eu-north-1.amazonaws.com/${data.trafficDebt.ipfsHash}`
            );
          }
        } else {
          setIsTrafficDebtInfoExists(false);
        }
      }
    };
    getTrafficDebt();
  ...
  const addIPFS = async () => {
    const input = trafficDebtTableRef.current;
    if (input)
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (ipfsHash) {
      ...
      const res = await fetch("/api/user/trafficDebt/addBlockchain", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${getAccessTokenFromLocalStorage()}`,
  ...
  return (
    ...
    >
      {isTrafficDebtInfoExists ? (
        <>
          {trafficDebt?.trafficDebtInfos ? (
            <Box
              sx={{
            ...
            >
              <TrafficDebtInfoTable {...{ trafficDebt, trafficDebtTableRef }} />
              {isButtonVisible ? (
                <Button
        ...
        >
          ...
          >
            You don&apos;t have any Traffic Debt info
          </Typography>
        </Box>
      )}
