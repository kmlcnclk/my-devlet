...
import MilitaryInfos from './MilitaryInfos';
import {
  Button,
  Paper,
  Table,
...
import { createXLSXFileForMilitary } from '@/lib/xlsxFileCreators';
import { createCSVFileForMilitary } from '@/lib/csvFileCreators';
import { createJSONFileForMilitary } from '@/lib/jsonFileCreators';

function MilitaryBackground() {
  const [userId, setUserId] = useState<string>('');
  const [militaryInfos, setMilitaryInfos] = useState<any>([]);
  ...
  useEffect(() => {
    if (typeof window != 'undefined') {
      const { url: xlsxURL } = createXLSXFileForMilitary();
      const { url: csvURL } = createCSVFileForMilitary();
      const { url: jsonURL } = createJSONFileForMilitary();
  ...
  return (
    ...
    >
      ...
      {militaryInfos.length > 0 ? (
        ...
        >
          ...
          >
            ...
            <TableBody>
              {militaryInfos.map((military: any, index: any) => (
                <TableRow key={index}>
      ...
      <Upload
        ...
        whichProperty="military"
        setFileData={setMilitaryInfos}
      />
      <Typography
        sx={{
          color: '#666',
