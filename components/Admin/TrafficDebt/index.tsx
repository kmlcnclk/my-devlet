...
import TrafficDebtInfos from "./TrafficDebtInfos";
import {
  Button,
  Paper,
  Table,
...
import { createXLSXFileForTrafficDebt } from "@/lib/xlsxFileCreators";
import { createCSVFileForTrafficDebt } from "@/lib/csvFileCreators";
import { createJSONFileForTrafficDebt } from "@/lib/jsonFileCreators";
...
function TrafficDebt() {
  ...
  const [trafficDebtInfos, setTrafficDebtInfos] = useState<any>([]);
  ...
  useEffect(() => {
    if (typeof window != "undefined") {
      const { url: xlsxURL } = createXLSXFileForTrafficDebt();
      const { url: csvURL } = createCSVFileForTrafficDebt();
      const { url: jsonURL } = createJSONFileForTrafficDebt();
  ...
  return (
    ...
    >
      ...
      {trafficDebtInfos.length > 0 ? (
        ...
        >
          ...
          >
            ...
            <TableBody>
              {trafficDebtInfos.map((trafficDebt: any, index: any) => (
                <TableRow key={index}>
      ...
      <Upload
        ...
        whichProperty="trafficDebt"
        setFileData={setTrafficDebtInfos}
      />
      <Typography
        sx={{
          color: "#666",
