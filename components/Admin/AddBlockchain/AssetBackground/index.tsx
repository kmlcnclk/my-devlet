...
function AssetBackground() {
  ...
  const [isAssetSelected, setIsAssetSelected] = useState<boolean>(false);
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (assetBackground) {
      ...
      if (ipfsHash) {
        ...
        const res = await fetch('/api/admin/assetBackground/addBlockchain', {
          method: 'POST',
          headers: {
      ...
      toast.info('Asset Background could not found');
    }
  };

  const getAsset = async () => {
    const res = await fetch(
      `/api/admin/assetBackground/getById?id=${assetId}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!res.ok) {
      ...
      toast.success('Asset is successfully found');
    }
  };
        ...
        >
          ...
          >
            ...
            <Box
              component="input"
              required
              disabled={Boolean(assetId) && isAssetSelected}
              value={assetId}
          ...
          <Button
            onClick={() => {
              if (assetId) {
                ...
                toast.info('You have to enter an asset id');
              }
            }}
            type="button"
            sx={{
