...
function CriminalRecord() {
  ...
  const [criminalRecordId, setCriminalRecordId] = useState<string>('');
  const [userId, setUserId] = useState<string>('');
  const [isCriminalRecordSelected, setIsCriminalRecordSelected] =
  ...
  const getCriminalRecord = async () => {
    const res = await fetch(
      `/api/admin/criminalRecord/getById?id=${criminalRecordId}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        ...
        >
          ...
          >
            ...
            <Box
              component="input"
              required
              disabled={Boolean(criminalRecordId) && isCriminalRecordSelected}
              value={criminalRecordId}
              onChange={(e: any) => setCriminalRecordId(e.target.value)}
              sx={{
                height: '40px',
                width: '100%',
                bgcolor: '#F8F9F8',
          ...
          <Button
            onClick={() => {
              if (criminalRecordId) {
                setIsCriminalRecordSelected(true);
                getCriminalRecord();
              } else {
                toast.info('You have to enter an criminalRecord id');
              }
            }}
            type="button"
            sx={{
