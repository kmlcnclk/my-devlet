...
function FamilyTreeBackground() {
  ...
  const [isFamilyTreeSelected, setIsFamilyTreeSelected] =
    ...
    if (familyTreeBackground) {
      ...
      if (ipfsHash) {
        ...
        const res = await fetch(
          '/api/admin/familyTreeBackground/addBlockchain',
          {
            method: 'POST',
            headers: {
      ...
      toast.info('Family Tree Background could not found');
    }
  };

  const getFamilyTree = async () => {
    const res = await fetch(
      `/api/admin/familyTreeBackground/getById?id=${familyTreeId}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!res.ok) {
      ...
      toast.success('Family Tree is successfully found');
    }
  };
        ...
        >
          ...
          >
            ...
            <Box
              component="input"
              required
              disabled={Boolean(familyTreeId) && isFamilyTreeSelected}
              value={familyTreeId}
              onChange={(e: any) => setFamilyTreeId(e.target.value)}
              sx={{
                height: '40px',
                width: '100%',
                bgcolor: '#F8F9F8',
          ...
          <Button
            onClick={() => {
              if (familyTreeId) {
                setIsFamilyTreeSelected(true);
                getFamilyTree();
              } else {
                toast.info('You have to enter an familyTree id');
              }
            }}
            type="button"
            sx={{
