...
import {
  ...
  TrafficDebtReturnType,
} from "@/types/TrafficDebt";
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (trafficDebt) {
      ...
      if (ipfsHash) {
        ...
        const res = await fetch("/api/admin/trafficDebt/addBlockchain", {
          method: "POST",
          headers: {
      ...
      toast.info("TrafficDebt  could not found");
    }
  };

  const getTrafficDebt = async () => {
    const res = await fetch(
      `/api/admin/trafficDebt/getByUserId?userId=${userId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!res.ok) {
      ...
      toast.success("TrafficDebt is successfully found");
    }
  };
