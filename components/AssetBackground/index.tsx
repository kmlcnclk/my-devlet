...
import {
  AddBlockChainType,
  AssetBackgroundReturnType,
} from '@/types/AssetBackground';
...
function AssetBackground() {
  const [assetBackground, setAssetBackground] =
    useState<AssetBackgroundReturnType | null>(null);
  ...
  const [isAssetInfoExists, setIsAssetInfoExists] = useState(true);
  ...
  useEffect(() => {
    const getAssetBackground = async () => {
      const res = await fetch('/api/user/assetBackground/getByUserId', {
        method: 'GET',
        headers: {
      ...
      if (!res.ok) {
        ...
        if (data.assetBackground) {
          setAssetBackground(data.assetBackground);
          if (data.assetBackground?.ipfsHash) {
            setIsButtonVisible(false);
            // setIPFSURL(`https://ipfs.io/ipfs/${data.assetBackground.ipfsHash}`);
            setIPFSURL(
              `https://neu-my-devlet.s3.eu-north-1.amazonaws.com/${data.assetBackground.ipfsHash}`
            );
          }
        } else {
          setIsAssetInfoExists(false);
        }
      }
    };
    getAssetBackground();
  ...
  const addIPFS = async () => {
    const input = assetTableRef.current;
    if (input)
  ...
  const addBlockChain = async (e: any) => {
    ...
    if (ipfsHash) {
      const addBlockchainData: AddBlockChainType = {
        id: assetBackground?._id,
        smartContract: smartContract,
        ipfsHash,
      };

      const res = await fetch('/api/user/assetBackground/addBlockchain', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${getAccessTokenFromLocalStorage()}`,
      ...
      {isAssetInfoExists ? (
        <>
          {assetBackground?.assetInfos ? (
            ...
            >
              <AssetInfoTable {...{ assetBackground, assetTableRef }} />
              {isButtonVisible ? (
                <Button
