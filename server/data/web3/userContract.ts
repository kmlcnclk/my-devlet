...
export default class UserContract {
  ...
  public async setFamilyTreeRecord(
    from: string,
    userId: string,
    genders: string[],
    ...
    try {
      const mintTx = await this._contract.methods.setFamilyTreeRecord(
        userId,
        genders,
  ...
  public async getFamilyTreeRecords(
    from: string,
    userId: string
  ): Promise<void> {
    try {
  ...
  public async setSubscriptionTransactionRecord(
    from: string,
    userId: string,
    ...
    try {
      const mintTx =
        await this._contract.methods.setSubscriptionTransactionRecord(
          userId,
          subscriptionTypes,
          companyNames,
  ...
  public async getSubscriptionTransactionRecords(
    from: string,
    userId: string
  ): Promise<void> {
    try {
      const mintTx = await this._contract.methods
        .getSubscriptionTransactionRecords(userId)
        .call();
    } catch (err: any) {
      if (err.error.message.indexOf("insufficient funds") != -1) {
        throw new CustomError("Web3 JS Error", "Insufficient funds", 500);
  ...
  public async setTrafficDebtRecord(
    from: string,
    userId: string,
    debtPayers: string[],
    debtAmounts: number[],
    ...
    try {
      const mintTx = await this._contract.methods.setTrafficDebtRecord(
        userId,
        debtPayers,
        debtAmounts,
        expiryDates,
  ...
  public async getTrafficDebtRecords(
    from: string,
    userId: string
  ): Promise<void> {
    try {
      const mintTx = await this._contract.methods
        .getTrafficDebtRecords(userId)
        .call();
    } catch (err: any) {
      if (err.error.message.indexOf("insufficient funds") != -1) {
        throw new CustomError("Web3 JS Error", "Insufficient funds", 500);
  ...
  public async setPlaceOfResidenceRecord(
    from: string,
    userId: string,
    names: string[],
    surnames: string[],
    ...
    try {
      const mintTx = await this._contract.methods.setPlaceOfResidenceRecord(
        userId,
        names,
        surnames,
        typeOfAddress,
  ...
  public async getPlaceOfResidenceRecords(
    from: string,
    userId: string
  ): Promise<void> {
    try {
      const mintTx = await this._contract.methods
        .getPlaceOfResidenceRecords(userId)
        .call();
    } catch (err: any) {
      if (err.error.message.indexOf("insufficient funds") != -1) {
        throw new CustomError("Web3 JS Error", "Insufficient funds", 500);
