...
async function handler(req: NextApiRequestWithUser, res: NextApiResponse) {
  if (req.method === "PUT") {
    try {
      ...
      await placeOfResidenceModel.findOneAndUpdate(
        {
          userId: placeOfResidenceData.userId,
        },
        {
          placeOfResidenceInfos: placeOfResidenceData.placeOfResidenceInfos,
          ipfsHash: "",
        }
      );

      return res.status(200).json({
        message: "Place of Residence successfully updated",
      });
    } catch (err: any) {
      if (err.status) {
