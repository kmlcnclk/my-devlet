...
async function handler(req: NextApiRequestWithUser, res: NextApiResponse) {
  if (req.method === 'PUT') {
    try {
      ...
      await familyTreeBackgroundModel.findOneAndUpdate(
        {
          userId: familyTreeBackgroundData.userId,
        },
        {
          familyTreeInfos: familyTreeBackgroundData.familyTreeInfos,
          ipfsHash: '',
        }
      );

      return res.status(200).json({
        message: 'Family Tree Background successfully updated',
      });
    } catch (err: any) {
      if (err.status) {
